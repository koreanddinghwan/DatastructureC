...
void min_heap_push_back(t_vector *vec, t_data data)
{
	...
	i = vec_size(vec) - 1;
	for (;i != 0 && (vec_data(vec)[(i - 1) / 2].weight > vec_data(vec)[i].weight); i = (i - 1) / 2)
	{
...
void min_heapify(t_vector *vec, int i)
{
	...
	if (left < vec_size(vec) && vec_data(vec)[left].weight < vec_data(vec)[i].weight)
		smallest = left;
	if (right < vec_size(vec) && vec_data(vec)[right].weight < vec_data(vec)[smallest].weight)
		smallest = right;
	if (smallest != i)
	{
